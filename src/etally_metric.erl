...
-spec send_event(Ls, LBBindings, Payload, Node) -> ok when
      Ls         :: [binary()],
      LBBindings :: [{binary(), binary()}],
      Payload    :: pos_integer(),
      Node       :: atom().

send_event(Ls, LBBindings, Payload, Node) ->
    etally_utils:cast_cnode({metric_event, Ls, LBBindings, Payload}, Node).


-spec send_event_bootstrap(Ls, LBBindings, Ts, Payload, Node) -> ok when
      Ls         :: [binary()],
      LBBindings :: [{binary(), binary()}],
      Ts         :: pos_integer(),
      Payload    :: pos_integer(),
...
send_event_bootstrap(Ls, LBBindings, Ts, Payload, Node) ->
    etally_utils:cast_cnode({metric_event_tz, Ls, LBBindings, Ts, Payload}, Node).
